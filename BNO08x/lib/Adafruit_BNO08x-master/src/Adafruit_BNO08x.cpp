...
bool Adafruit_BNO08x::begin_I2C(uint8_t i2c_address, TwoWire *wire,
                                int32_t sensor_id) {
  if (i2c_dev) {
    Serial.println("yeet old i2c");
    delete i2c_dev; // remove old interface
  }

  i2c_dev = new Adafruit_I2CDevice(i2c_address, wire);

  Serial.println("lol allo");
...
bool Adafruit_BNO08x::_init(int32_t sensor_id) {
  ...
  Serial.println("passed sh2");

  // Check connection partially by getting the product id's
  memset(&prodIds, 0, sizeof(prodIds));
  Serial.println("passed memset");
